...
const Carousel: React.FC<CarouselProps> = ({ selected, setSelected, images }) => {
  ...
  const centerImageIndex = (scrollLeft: number, imageWidth: number) => {
    const centerPosition = scrollLeft + (carouselRef.current?.clientWidth || 0) / 2;
  ...
  const handleScrollEnd = () => {
    if (carouselRef.current) {
      ...
      const imageWidth = carouselRef.current.scrollWidth / duplicatedImages.length;
      ...
      const targetScrollLeft = centerIndex * imageWidth - (carouselRef.current.clientWidth - imageWidth) / 2;
  ...
  useEffect(() => {
    ...
    if (carousel) {
      ...
      const targetScrollLeft = initialIndex * imageWidth - (carousel.clientWidth - imageWidth) / 2;
  ...
  return (
    <div className="relative w-full max-w-4xl overflow-hidden mt-8 sm:mt-10 lg:mt-12" style={{ height: "40vh" }}>
      <div
        ref={carouselRef}
...
{duplicatedImages.map((image, index) => {
  ...
  return (
    ...
    >
      ...
      />
      {isSelected && (
        <div className="mt-1 text-center"> {/* Reduce el margen superior */}
          <p className="text-white text-sm font-medium">{image.name}</p> {/* Texto más pequeño */}
        </div>
      )}
    </div>
  );
